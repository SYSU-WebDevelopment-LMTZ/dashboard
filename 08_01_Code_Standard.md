...
- Vuex目录 （状态树配置）
- Router目录（路由配置）
- Pages目录 （放置主路由组件 注意命名规范）
- Common目录 （放置静态文件）
- Config目录 （全局配置项，路由拦截，报错信息，等枚举信息）
- Api目录 （ 相关全局请求调用配置、axios、jsonp ）
- Base目录 （可复用功能性组件）
- Basic目录 （可复用样式展示组件）
- Components目录（基础组件：样式组件等 注意组件分类规范）
- 核心：在每个目录下面留一份TEXT或MARKDOWN文档，用中文批注好每个组件文件夹的功能及使用，每次在当前目录下新建或更新当前目录时，做好批注 
...
- 统一采用axios方式
- 登陆拦截方式
- 参数传递：配合query string 实现对象到uri的格式化
- GET请求规范
- POST请求规范
- 其他类型请求规范
- 避免拼接uri字符串参数情况的产生
...
### VUE模板语句使用规范
- 条件 （v-if , v-else, v-if-else, v-show）合理使用 v-if 及 v-show
- 循环 （v-for、 :key）
- 样式 （:style、:class）
- 事件绑定 （v-on、@）
- props传递 （加类型判断，不能直接数组接收）
- 其他优化细节（v-text、v-html、v-once）

### 组件模板开发
- 尽可能语义化标签，使结构更加信息
- 大段功能模块明确中文注释
- 在代码结构很长的情况下，用分割线注释批注
- 模板语法中的逻辑在methods中处理
- Tab缩进，四个空格，层次分明

### 路由定义
- 全局路由钩子拦截
- 懒加载中AMD规范require和ES6中的import的选择
- 拦截条件语句筛选
- 路由名称具有可读性
...
- 组件文件夹命名：
  - 按照功能英文命名，过长则用 ” - ” 连接
  - 其内部的组件名称和样式名称与文件夹同名
  - 其风格与微信小程序组件文件夹命名保持一致
- 静态资源文件夹static命名：
  - 英文命名，过长则用 ” - ” 连接
  - 其主目录创建一个解释文件，在这个解释文件中使用中文批注好每个目录的内容，以及每个目录正在被哪些组件调用
- 图片文件命名：
  - 如果是精灵图，则需按功能命名
  - 如果是列表渲染图片，则需要按照1-100编号命名
  - 如果是ICON图片，则需要添加 ”icon-”前缀

### 注释规范
- TEMPLATE结构内容注释
  - 大区块之间有回车换行，且有有单独的中文注释
- STYLUS注释
  - 每个大区块的样式的有单独的中文注释
  - 每个大区块样式之间回车换行
  - 在STYLUS自定义函数库文件类似于mixin.styl，则需要对每个语言函数进行单独的批注，或者一些功能类似的语言函数可以根据功能分类注释
- SCRIPT注释
  - 尽可能多用单行注释，注释需要与被注释的地方对齐
  - 方法注释，filters里面的过滤器注释说明功能